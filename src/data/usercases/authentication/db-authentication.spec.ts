import { AuthenticationModel, LoadAccountByEmailRepository, HashComparer, TokenGenerator, UpdateAccessTokenRepository, AccountModel, DbAuthentication } from './db-authentication-protocols'
...
const makeSut = (): SutTypes => {
  ...
  const sut = new DbAuthentication(loadAccountRepositoryStub, hashComparerStub, tokenGeneratorStub, updateAccessTokenRepositoryStub)
  return {
    sut,
...
describe('DbAuthentication UseCase', () => {
  ...
  test('Should throw if UpdateAccessTokenRepository throws exception', async () => {
    const { sut, updateAccessTokenRepositoryStub } = makeSut()
    jest.spyOn(updateAccessTokenRepositoryStub, 'update').mockReturnValueOnce(new Promise((resolve, reject) => reject(new Error('any error'))))
    const promise = sut.auth(makeFakeAuthentication())
    // eslint-disable-next-line @typescript-eslint/no-floating-promises
    await expect(promise).rejects.toThrow()
  })
